...
observation_mesh = VertexOnlyMesh(mesh, observation_configs["sparse"])  # 使用稀疏观测配置
...
obs_times = [0, len(q_obs_series)//3, 2*len(q_obs_series)//3, -1] #ob times initial time
...
print((err_prior-err_opt)/err_prior)
...
print(f"Observation improvement ratio: {(obs_error_prior-obs_error_opt)/obs_error_prior:.6f}")
