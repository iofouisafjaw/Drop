...
V = FunctionSpace(mesh, "DG", 1)  # 不连续Galerkin空间
W = VectorFunctionSpace(mesh, "CG", 1)  # 速度场空间
x = SpatialCoordinate(mesh)

# 定义速度场
velocity = as_vector((u_vel,))
...
# DG时间积分函数（Forward Euler）
def integrate_forward_dg(q_init, nt_steps):
    """从初始条件积分到指定时间步（DG格式）"""
    q = Function(V)
    q.assign(q_init)
    ...
    # DG格式的数值通量
    n = FacetNormal(mesh)
    un = 0.5 * (dot(u, n) + abs(dot(u, n)))  # Lax-Friedrichs通量
    
    for n_step in range(nt_steps):
        v = TestFunction(V)
        
        # DG格式的弱形式
        F = (q - q_init)/dt*v*dx + q*div(v*u)*dx
        
        # 边界条件（周期性）
        F -= conditional(dot(u, n) < 0, v*dot(u, n)*q_init, 0.0)*ds
        F -= conditional(dot(u, n) > 0, v*dot(u, n)*q, 0.0)*ds
...
def objective_function(q0, observations, obs_times, obs_positions, q_b):
    ...
    for n_step in range(nt + 1):
        ...
        if n_step < nt:
            ...
            # DG格式的弱形式
            F = (q - q0)/dt*v*dx + q*div(v*u)*dx
            
            # 边界条件（周期性）
            F -= conditional(dot(u, n) < 0, v*dot(u, n)*q0, 0.0)*ds
            F -= conditional(dot(u, n) > 0, v*dot(u, n)*q, 0.0)*ds
...
def main():
    ...
    true_trajectory = integrate_forward_dg(q_true_init, nt)
    ...
    background_trajectory = integrate_forward_dg(q_b_copy, nt)
    ...
    initial_trajectory = integrate_forward_dg(q0_copy, nt)
    ...
    optimal_trajectory = integrate_forward_dg(opt_q0_copy, nt)
    ...
    # 12. 保存结果
    print("12. 保存结果...")
    File("dg_optimal_initial_condition.pvd").write(opt_q0)
    File("dg_true_initial_condition.pvd").write(q_true_init)
    File("dg_background_field.pvd").write(q_b)
    File("dg_initial_guess.pvd").write(q0)
    
    print("=== DG对流方程 4D-Var 完成 ===")
