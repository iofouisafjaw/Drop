...
def generate_data(V, x, q_true0, n_samples=1000, purpose="vae"):
    """生成1D训练数据"""
    print(f"Generating {n_samples} 1D training samples...")
    training_data = []
    n_samples = 200 if purpose=="vae" else 200
    for i in range(n_samples):
        # 添加随机噪声
        noise_amplitude = 0.05 if purpose == "vae" else 0.03 
        ...
        amp1 = np.random.uniform(0.05, 0.15) if purpose == "vae" else np.random.uniform(0.02, 0.08)
        amp2 = np.random.uniform(0.05, 0.15) if purpose == "vae" else np.random.uniform(0.02, 0.08)
...
def vae_4dvar_1d(mesh, V, q_true0, qb, y_obs_series, H, forward_solve):
    ...
    training_data = generate_data(V, x, q_true0, n_samples=800, purpose="vae")
    ...
    q_series = forward_solve(q_decoded, annotate_tf=True)
...
def compute_U(V, x, q_true0, n_ensemble=200):
    print(f"Generating {n_ensemble} background ensemble members for U matrix...")
    ensemble_data = generate_data(V, x, q_true0, n_samples=800, purpose="pca") 
    mean_bg = np.mean(ensemble_data, axis=0)

    centered_data = ensemble_data - mean_bg
    n_components = min(20, n_ensemble-1)  
    pca = PCA(n_components=n_components)
    pca.fit(centered_data)

    explained_var = pca.explained_variance_
    U = pca.components_.T * np.sqrt(explained_var)  
    
    print(f"U matrix shape: {U.shape}")
    print(f"Explained variance ratio: {np.sum(pca.explained_variance_ratio_):.4f}")
    
    return U, mean_bg, pca.explained_variance_ratio_


def traditional_4dvar(qb, y_obs_series, H, forward_solve, U_matrix, mean_bg):
    print("\n" + "="*50)
    print("Starting Traditional 4D-Var with U transform...")
    print("="*50)
    
    continue_annotation()
    
    # 潜在空间维度
    latent_dim = U_matrix.shape[1]
    
    # 初始化潜在变量z（标准正态分布）
    mesh = qb.function_space().mesh()
    z_spaces = [FunctionSpace(mesh, "R", 0) for _ in range(latent_dim)]
    z_funcs = [Function(Z_space, name=f"z_{i}") for i, Z_space in enumerate(z_spaces)]
    
    # 初始化z为0（标准正态均值）
    for z_func in z_funcs:
        z_func.assign(0.0)
    
    def linear_transform_to_physical(z_funcs, qb_mean):
        """从潜在空间变换到物理空间: q0 = qb_mean + U*z"""
        z_array = np.array([z_func.dat.data[0] for z_func in z_funcs])
        
        # 线性变换
        physical_perturbation = U_matrix @ z_array  # [n_spatial,]
        
        # 构造物理空间初始条件
        q0_physical = Function(qb.function_space())
        q0_physical.dat.data[:] = qb_mean + physical_perturbation
    ...
    J_background_terms = []
    for z_func in z_funcs:
        background_term = 0.5 * z_func * z_func * dx
        J_background_terms.append(background_term)
    ...
    # === 前向模拟和观测项 ===
    q0_initial = linear_transform_to_physical(z_funcs, mean_bg)
    q_series = forward_solve(q0_initial, annotate_tf=True)
    J_obs = 0.0
    sigma2 = 1e-4
    for qk, yk in zip(q_series, y_obs_series):
        diff = H(qk) - yk
    ...
    # 总cost function
    J_total = J_background_assembled + J_obs
    ...
    # 优化
    print("Optimizing in latent space...")
    opt_z_funcs = minimize(rf, method="L-BFGS-B", 
                          options={"disp": True, "maxiter": 30}, 
                          derivative_options={'riesz_representation':'l2'})
    
    # 变换回物理空间得到最优初始条件
    opt_q0_physical = linear_transform_to_physical(opt_z_funcs, mean_bg)
    
    print("Traditional 4D-Var with U transform completed!")
    print("="*50)
    
    return opt_q0_physical
...
def traditional_4dvar_1d(qb, y_obs_series, H, forward_solve):
    ...
    q_series = forward_solve(q0, annotate_tf=True)
...
def main():
    ...
    def forward_solve(q0, annotate_tf):
        q = Function(V, name="q")
        q.assign(q0)
        ...
        while t < float(T) - 0.5*float(T/nt):
            ...
            if step % output_freq == 0:
                series.append(q.copy(deepcopy=True, annotate=annotate_tf))
        
        return series
    print("Generating observations...")
    q_true_series = forward_solve(q_true0, annotate_tf=False)
    y_obs_series = [H(qk) for qk in q_true_series]
    for y in y_obs_series:
        noise = 0.0 * np.random.normal(0, 0.01, y.dat.data.shape)  
        y.dat.data[:] += noise
    
    print("\nComputing U transformation matrix...")
    U_matrix, mean_bg, variance_ratio = compute_U(V, x, q_true0, n_ensemble=200)

    q_vae_optimal, vae_optimizer = vae_4dvar_1d(mesh, V, q_true0, qb, y_obs_series, H, forward_solve)
    opt_q0_traditional_4d = traditional_4dvar(qb, y_obs_series, H, forward_solve, U_matrix, mean_bg)
    opt_q0 = traditional_4dvar_1d(qb, y_obs_series, H, forward_solve)
    print(1111)
    # 计算误差
    plot_1d_comparison(q_true0, qb, opt_q0_traditional_4d, q_vae_optimal, obs_points, mesh)
    err_vae = errornorm(q_true0, q_vae_optimal, 'L2')
    err_trad_4d = errornorm(q_true0, opt_q0_traditional_4d, 'L2')
    err_tra = errornorm(q_true0, opt_q0, 'L2')
    print(f"\nFinal L2 Error Comparison:")
    print(f"VAE-4DVar L2 error:       {err_vae:.6f}")
    print(f"Traditional 3D-Var error: {err_trad_4d:.6f}")
    print(f"Traditional 3D-Var error: {err_tra:.6f}")
...
def plot_1d_comparison(q_true0, qb, opt_q0_traditional, q_vae_optimal, obs_points, mesh):
    ...
    plt.tight_layout()
    plt.savefig("111111111111111.png", dpi=300, bbox_inches='tight')
    plt.show()
