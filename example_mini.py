#!/usr/bin/env python3
"""
Simple 4D-Var following handwritten steps - no classes
按照手写笔记的步骤一步一步实现
"""
...
V = FunctionSpace(mesh, "DQ", 1)  # scalar field space
W = VectorFunctionSpace(mesh, "CG", 1)  # vector field space
...
T = 2*math.pi  # total time (one full rotation)
dt = T/600.0   # time step
dtc = Constant(dt)
q_in = Constant(1.0)  # inflow value

print("=== Step 4: Set up DG formulation ===")
phi = TestFunction(V)
dq_trial = TrialFunction(V)
a = phi * dq_trial * dx  # mass matrix
...
def create_background():
    # Different from true (this is our "first guess")
...
print("=== Step 7: Define forward solver function ===")
def solve_forward(q0, store_freq=20):
    """Forward solve from initial condition q0"""
    q = Function(V, name="q")
    q.assign(q0)
...
print("=== Step 10: Test cost function ===")
...
print("=== Step 11: Simple optimization ===")
print("Starting optimization from q_background...")
...
for iteration in range(max_iterations):
    # Generate random perturbation using Fourier modes
...
try:
    ...
    plt.tight_layout()
    plt.savefig('Simple_4DVar_Step_by_Step.png', dpi=150, bbox_inches='tight')
    plt.show()
    
except Exception as e:
    print(f"Plotting error: {e}")

print("\n=== 4D-Var completed following handwritten steps! ===")